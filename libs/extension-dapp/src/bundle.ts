...
let web3WindowExtension: string | null = null;

export function setWindowExtension(extensionName: string) {
  web3WindowExtension = extensionName;
}

function getWindowExtensions(
  originName: string
): Promise<[InjectedExtensionInfo, Injected | void][]> {
  if (web3WindowExtension) {
    const injectedWeb3 = win.injectedWeb3
      ? win.injectedWeb3[web3WindowExtension]
      : null;
    if (injectedWeb3) {
      const name = web3WindowExtension;
      const { enable, version } = injectedWeb3;
      return Promise.all([
        Promise.all([
...
// enables the provider set by calling `setWindowExtension`
export function web3EnableOne(
  extensionName: string,
  originName: string,
  compatInits: (() => Promise<boolean>)[] = []
) {
  setWindowExtension(extensionName);
  return web3Enable(originName, compatInits);
}
      ...
      initCompat.then(() =>
        getWindowExtensions(originName)
          .then((values): InjectedExtension[] =>
            values
              .filter(
                (value): value is [InjectedExtensionInfo, Injected] =>
                  !!value[1]
              )
