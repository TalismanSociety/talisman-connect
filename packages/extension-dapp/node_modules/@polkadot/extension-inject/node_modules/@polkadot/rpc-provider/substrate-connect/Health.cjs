...
function healthChecker() {
  ...
  return {
    ...
    sendJsonRpc: request => {
      (0, _util.assert)(sendJsonRpc, 'setSendJsonRpc must be called before sending requests');
    ...
    start: healthCallback => {
      (0, _util.assert)(checker === null, "Can't start the health checker multiple times in parallel");
      (0, _util.assert)(sendJsonRpc, 'setSendJsonRpc must be called before starting the health checks');
...
class InnerChecker {
  ...
  responsePassThrough = jsonRpcResponse => {
    ...
    if (parsedResponse.id) {
      ...
      (0, _util.assert)(id.startsWith('extern:'), 'State inconsistency in health checker');
  ...
  startSubscription = () => {
    (0, _util.assert)(!this.#currentSubunsubRequestId && !this.#currentSubscriptionId, 'Internal error in health checker');
