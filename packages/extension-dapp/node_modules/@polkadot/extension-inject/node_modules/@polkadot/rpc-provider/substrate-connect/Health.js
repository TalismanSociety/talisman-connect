...
export function healthChecker() {
  ...
  return {
    ...
    sendJsonRpc: request => {
      assert(sendJsonRpc, 'setSendJsonRpc must be called before sending requests');
    ...
    start: healthCallback => {
      assert(checker === null, "Can't start the health checker multiple times in parallel");
      assert(sendJsonRpc, 'setSendJsonRpc must be called before starting the health checks');
...
class InnerChecker {
  ...
  startSubscription = () => {
    assert(!this.#currentSubunsubRequestId && !this.#currentSubscriptionId, 'Internal error in health checker');
