...
export class ScProvider {
  ...
  async connect() {
    assert(!this.isConnected, 'Already connected!'); // it could happen that after emitting `disconnected` due to the fact taht
    ...
    const client = this.#sharedSandbox ? scClients.get(this.#sharedSandbox) : createScClient();
  ...
  async send(method, params) {
    assert(this.isConnected, 'Provider is not connected'); // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
  ...
  async subscribe(type, method, params, callback) {
    assert(subscriptionUnsubscriptionMethods.has(method), `Unsupported subscribe method: ${method}`);
